...
const logger = (0, logger_1.createLogger)('ai-routes');
const aiRoutes = (fastify, options, done) => {
    /**
     * POST /api/ai/generate - Generate AI pixel art image
     */
    fastify.post('/generate', {
        schema: {
            body: api_1.AIGenerateRequestSchema,
            response: {
        ...
        try {
            const { prompt, mode, width, height, colorLimit, referenceImageId, referenceImageData, seed, enableDithering, quantizationMethod, } = body;
            let aiImageUrl;
            let aiImageBuffer;
            ...
            if (mode === 'new') {
                ...
                if (referenceImageId) {
                    ...
                    const [referenceAsset] = await connection_1.db
                        .select()
                        .from(schema_1.assets)
                        .where((0, drizzle_orm_1.eq)(schema_1.assets.id, referenceImageId))
                        .limit(1);
                    if (!referenceAsset) {
                ...
                const results = await openai_1.openaiService.editImage({
            ...
            const processedImage = await imageProcessing_1.imageProcessingService.processAIImage(aiImageBuffer, {
                width,
                height,
                colorLimit,
            ...
            const thumbnail = await imageProcessing_1.imageProcessingService.createThumbnail(processedImage.buffer, 128);
            // Step 4: Upload processed image to storage
            const filename = `ai-generated-${requestId}`;
            const uploadResults = await Promise.all([
                storage_1.storageService.uploadImage(processedImage.buffer, `${filename}.png`, 'image/png'),
                storage_1.storageService.uploadImage(thumbnail, `${filename}-thumb.png`, 'image/png'),
            ]);
            const [processedUrl, thumbUrl] = uploadResults;
            // Step 5: Save to database
            const [savedAsset] = await connection_1.db.insert(schema_1.assets).values({
                userId,
                type: 'ai',
    ...
    fastify.post('/variations', {
        schema: {
            body: api_1.AIVariationsRequestSchema,
            response: {
                200: {
                    type: 'array',
                    items: api_1.AIGenerationResponseSchema,
                },
            },
        },
        preHandler: [
        ...
        try {
            ...
            if (assetId) {
                ...
                const [sourceAsset] = await connection_1.db
                    .select()
                    .from(schema_1.assets)
            ...
            const variations = await openai_1.openaiService.generateVariations({
                image: sourceBuffer,
                n: count,
                size: '512x512',
                responseFormat: 'url',
            ...
            const results = await Promise.all(variations.map(async (variation, index) => {
                if (!variation.url) {
                ...
                // Process variation
                const processed = await imageProcessing_1.imageProcessingService.processAIImage(variationBuffer, {
                    width,
                    height,
                    colorLimit,
                ...
                // Create thumbnail
                const thumbnail = await imageProcessing_1.imageProcessingService.createThumbnail(processed.buffer, 128);
                // Upload to storage
                ...
                const [processedUrl, thumbUrl] = await Promise.all([
                    storage_1.storageService.uploadImage(processed.buffer, `${filename}.png`, 'image/png'),
                    storage_1.storageService.uploadImage(thumbnail, `${filename}-thumb.png`, 'image/png'),
                ]);
                // Save to database
                const [savedAsset] = await connection_1.db.insert(schema_1.assets).values({
                    ...
                    palette: processed.palette.map(color => `#${color.r.toString(16).padStart(2, '0')}${color.g.toString(16).padStart(2, '0')}${color.b.toString(16).padStart(2, '0')}`),
                    metadata: {
                        colorLimit,
                        quantizationMethod: 'median-cut',
