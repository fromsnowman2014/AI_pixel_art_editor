...
const logger = (0, logger_1.createLogger)('image-processing');
class ImageProcessingService {
    /**
     * Process AI-generated image for pixel art: quantize + resize
     */
    async processAIImage(inputBuffer, options) {
        ...
        try {
            ...
            const { quantizedBuffer, palette } = await this.quantizeColors(intermediateBuffer.data, intermediateBuffer.info.width, intermediateBuffer.info.height, options.colorLimit, options.quantizationMethod || 'median-cut');
            // Step 4: Apply dithering if enabled
            let processedBuffer = quantizedBuffer;
            if (options.enableDithering) {
                processedBuffer = await this.applyFloydSteinbergDithering(quantizedBuffer, intermediateBuffer.info.width, intermediateBuffer.info.height, palette);
            }
            // Step 5: Final resize to target size using nearest-neighbor
            const finalBuffer = await (0, sharp_1.default)(processedBuffer, {
                raw: {
     ...
     */
    async quantizeColors(rgbaBuffer, width, height, targetColors, method = 'median-cut') {
        if (method === 'wu') {
     ...
     */
    async quantizeColorsMedianCut(rgbaBuffer, width, height, targetColors) {
     ...
     */
    async quantizeColorsWu(rgbaBuffer, width, height, targetColors) {
     ...
     */
    medianCutRecursive(colors, targetColors) {
    ...
    findClosestColor(targetColor, palette) {
        ...
        for (const paletteColor of palette) {
            const distance = Math.sqrt(Math.pow(targetColor.r - paletteColor.r, 2) +
     ...
     */
    async applyFloydSteinbergDithering(rgbaBuffer, width, height, palette) {
     ...
     */
    async createThumbnail(inputBuffer, maxSize = 128) {
     ...
     */
    async extractPalette(inputBuffer, maxColors = 256) {
        const image = (0, sharp_1.default)(inputBuffer);
