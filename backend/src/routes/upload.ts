...
const ProcessOptionsSchema = z.object({
  ...
  colorLimit: z.number().min(2).max(256).default(24),
  preserveAspectRatio: z.boolean().default(true),
  enableDithering: z.boolean().default(false),
  quantizationMethod: z.enum(['median-cut', 'wu']).default('median-cut'),
});

export const uploadRoutes: FastifyPluginCallback = (fastify, options, done) => {
  ...
  fastify.post('/image', {
    ...
    try {
      ...
      for await (const part of parts) {
        if (part.file) {
          ...
          if (part.fieldname === 'options') {
            try {
              processOptions = JSON.parse(part.value as string);
            } catch (error) {
      ...
      const processedImage = await imageProcessingService.processAIImage(
        imageFile.buffer,
        {
      ...
      const thumbnailImage = await imageProcessingService.processAIImage(
        processedImage.buffer,
        {
          width: 16,
          height: 16,
