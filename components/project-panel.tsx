...
export function ProjectPanel({ className }: { className?: string }) {
  ...
  const handleAiGenerate = async () => {
    ...
    debugLog('AI_GENERATION_START', 'Starting AI generation', {
      prompt: aiPrompt,
    ...
    try {
      ...
      debugLog('AI_HEALTH_CHECK', 'Checking backend health')
      const healthCheck = await api.health()
      
      if (!healthCheck.openaiKeyLoaded) {
        debugLog('AI_HEALTH_ERROR', 'OpenAI key not loaded on server')
        toast.error('AI service unavailable: Server key missing')
        return
      }

      if (healthCheck.services.openai !== 'healthy') {
        debugLog('AI_HEALTH_ERROR', 'OpenAI service unhealthy')
        toast.error('AI service temporarily unavailable')
        return
      }

      debugLog('AI_HEALTH_SUCCESS', 'Health check passed')

      // Step 2: Generate AI image
      debugLog('AI_GENERATE_REQUEST', 'Making generation request')
      ...
      debugLog('AI_GENERATE_SUCCESS', 'AI generation completed', {
      ...
      debugLog('AI_GENERATION_ERROR', 'AI generation failed', error)
  ...
  const loadImageToCanvas = async (imageUrl: string) => {
    debugLog('LOAD_IMAGE_START', 'Loading AI image to canvas', { imageUrl })
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      img.onload = () => {
        try {
          ...
          debugLog('LOAD_IMAGE_CANVAS', 'Extracted pixel data', {
          ...
          if (isEmpty && activeTab?.frames.length === 0) {
            debugLog('LOAD_IMAGE_NEW_FRAME', 'Creating new frame for empty project')
            addFrame(activeTabId)
          } else if (!isEmpty) {
            debugLog('LOAD_IMAGE_APPEND_FRAME', 'Appending new frame with AI image')
            addFrame(activeTabId)
          }
          ...
          debugLog('LOAD_IMAGE_SUCCESS', 'Successfully loaded AI image to canvas')
          resolve()
          
        } catch (error) {
          debugLog('LOAD_IMAGE_ERROR', 'Failed to process image', error)
          reject(error)
        }
      }
      
      img.onerror = () => {
        debugLog('LOAD_IMAGE_ERROR', 'Failed to load image from URL', { imageUrl })
        ...
        <div className="border-t border-gray-200 p-4">
          ...
          <div className="space-y-3">
            <textarea
              ...
              className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm resize-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 disabled:bg-gray-50 disabled:text-gray-500"
              rows={3}
